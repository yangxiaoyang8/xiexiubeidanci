...
(pushd "$ROOT_DIR/client" > /dev/null && npx expo export --platform web; popd > /dev/null) || error "前端构建失败"
info "==================== 前端构建完成！====================\n"

info "==================== 复制前端文件到server/dist ===================="
# 将前端静态文件复制到 server/dist 目录，以便生产环境可以托管
cp -r "$ROOT_DIR/client/dist" "$ROOT_DIR/server/dist/client-dist"
info "前端文件已复制到 server/dist/client-dist"
ls -la "$ROOT_DIR/server/dist/client-dist/"
info "==================== 复制完成！====================\n"
